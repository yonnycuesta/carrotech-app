...
@Component({
  ...
  templateUrl: './order-openend.component.html',
  styleUrl: './order-openend.component.scss'
})
export class OrderOpenendComponent implements AfterViewInit, OnInit {


  displayedColumns: string[] = ['code', 'staff_name', 'shift', 'statu',
    'comment', 'delivered_opt',
    'created_at', 'action'];
  ...
  getOrders() {
    try {
      this.sOrderMang.indexOpenend(this.managerId).subscribe({
        next: (resp: any) => {
