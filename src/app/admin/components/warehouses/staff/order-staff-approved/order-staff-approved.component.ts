...
@Component({
  selector: 'app-order-staff-approved',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './order-staff-approved.component.html',
  styleUrl: './order-staff-approved.component.scss'
})
export class OrderStaffApprovedComponent implements AfterViewInit, OnInit {
  ...
  getOrders() {
    try {
      this.sOrder.approvedStaff(this.managerId).subscribe({
        next: (resp: any) => {
  ...
  preparedAll() {
    ...
    try {
      ...
      Swal.fire({
        title: 'Confirmación',
        text: '¿Estás seguro de marcar como Preparada/Empaquetada todas las órdenes seleccionadas?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          ...
          this.sOrder.preparedAll(this.selectedIds).subscribe({
            next: (resp) => {
              this.showSuccess('Las órdenes han sido marcadas como Preparada/Empaquetada con éxito');
              this.selectedIds = [];
              this.ngOnInit();
            },
            error: (error) => {
              console.error('Error al marcar como Preparada/Empaquetada las órdenes:', error);
